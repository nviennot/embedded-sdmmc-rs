...
impl FatVolume {
        ...
        T: TimeSource,
    {
        match &self.fat_specific_info {
            ...
            FatSpecificInfo::Fat32(fat32_info) => {
                ...
                controller
                    .block_device
                    .read(&mut blocks, fat32_info.info_location, "read_info_sector").await
        ...
        T: TimeSource,
    {
        ...
        match &self.fat_specific_info {
            FatSpecificInfo::Fat16(_fat16_info) => {
                ...
                controller
                    .block_device
                    .read(&mut blocks, this_fat_block_num, "read_fat").await
                    .map_err(Error::DeviceError)?;
        ...
        controller
            .block_device
            .write(&blocks, this_fat_block_num).await
            .map_err(Error::DeviceError)?;
        Ok(())
    }
        ...
        T: TimeSource,
    {
        ...
        match &self.fat_specific_info {
            FatSpecificInfo::Fat16(_fat16_info) => {
                ...
                controller
                    .block_device
                    .read(&mut blocks, this_fat_block_num, "next_cluster").await
                    .map_err(Error::DeviceError)?;
    ...
    async fn find_entry_in_block<D, T>(
        &self,
        controller: &mut Controller<D, T>,
        fat_type: FatType,
    ...
    pub(crate) async fn delete_directory_entry<D, T>(
        &self,
        controller: &mut Controller<D, T>,
        dir: &Directory,
        name: &str,
        ...
        T: TimeSource,
    {
        ...
        match &self.fat_specific_info {
            FatSpecificInfo::Fat16(fat16_info) => {
                ...
                while let Some(cluster) = current_cluster {
                    for block in first_dir_block_num.range(dir_size) {
                        match self.delete_entry_in_block(controller, &match_name, block).await {
                            Err(Error::NotInBlock) => continue,
                            x => return x,
                        }
                    }
    ...
    async fn delete_entry_in_block<D, T>(
        &self,
        controller: &mut Controller<D, T>,
        match_name: &ShortFileName,
        block: BlockIdx,
        ...
        T: TimeSource,
    {
        ...
        let new_cluster = match self.find_next_free_cluster(controller, start_cluster, end_cluster).await
        {
            Ok(cluster) => cluster,
            Err(_) if start_cluster.0 > RESERVED_ENTRIES => {
                ...
                self.find_next_free_cluster(controller, Cluster(RESERVED_ENTRIES), end_cluster).await?
            }
            Err(e) => return Err(e),
        };
        self.update_fat(controller, new_cluster, Cluster::END_OF_FILE).await?;
        ...
        self.next_free_cluster =
            match self.find_next_free_cluster(controller, new_cluster, end_cluster).await {
        ...
        T: TimeSource,
    {
        ...
        self.update_fat(controller, cluster, Cluster::END_OF_FILE).await?;
        loop {
    ...
    D::Error: core::fmt::Debug,
{
    ...
    controller
        .block_device
        .read(&mut blocks, lba_start, "read_bpb").await
        .map_err(Error::DeviceError)?;
    let block = &blocks[0];
