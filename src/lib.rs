...
// These two features are for async support
    ...
    <D as BlockDevice>::Error: core::fmt::Debug,
{
    ...
    pub async fn get_volume(&mut self, volume_idx: VolumeIdx) -> Result<Volume, Error<D::Error>> {
        ...
        let (part_type, lba_start, num_blocks) = {
            ...
            self.block_device
                .read(&mut blocks, BlockIdx(0), "read_mbr").await
        ...
        let file = match mode {
            ...
            Mode::ReadWriteTruncate => {
                ...
                match &mut volume.volume_type {
                    VolumeType::Fat(fat) => {
                        fat.truncate_cluster_chain(self, file.starting_cluster).await?
                    }
                };
        ...
        match mode {
            Mode::ReadWriteCreate => {
                ...
                let entry = match &mut volume.volume_type {
                    VolumeType::Fat(fat) => {
                        fat.write_new_directory_entry(self, dir, file_name, att).await?
                    }
                };
    ...
    pub async fn read(
        ...
        while space > 0 && !file.eof() {
            ...
            let mut blocks = [Block::new()];
            self.block_device
                .read(&mut blocks, block_idx, "read").await
                .map_err(Error::DeviceError)?;
            let block = &blocks[0];
    ...
    pub async fn write(
        ...
        while written < bytes_to_write {
            ...
            let (block_idx, block_offset, block_avail) =
                match self.find_data_on_disk(volume, &mut current_cluster, file.current_offset).await {
                    Ok(vars) => {
                        debug!(
                    ...
                    Err(Error::EndOfFile) => {
                        ...
                        match &mut volume.volume_type {
                            VolumeType::Fat(ref mut fat) => {
                                if fat
                                    .alloc_cluster(self, Some(current_cluster.1), false).await
                                    .is_err()
                                {
                                    return Ok(written);
                                }
            ...
            match &mut volume.volume_type {
                VolumeType::Fat(fat) => {
                    ...
                    self.write_entry_to_disk(fat.get_fat_type(), &file.entry).await?;
                }
            }
        }
        Ok(written)
    ...
    async fn write_entry_to_disk(
        ...
        let mut blocks = [Block::new()];
        self.block_device
            .read(&mut blocks, entry.entry_block, "read").await
            .map_err(Error::DeviceError)?;
        let block = &mut blocks[0];
        ...
        self.block_device
            .write(&blocks, entry.entry_block).await
            .map_err(Error::DeviceError)?;
        Ok(())
    }
}
